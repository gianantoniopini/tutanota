...
export class SendMailModel {
	...
	_updateDraft(body: string, attachments: ?$ReadOnlyArray<Attachment>, draft: Mail): Promise<Mail> {
		return this._mailFacade
		           .updateDraft(this.getSubject(), body, this._senderAddress, this.getSenderName(), this.toRecipients(),
	...
	_createDraft(body: string, attachments: ?$ReadOnlyArray<Attachment>, mailMethod: MailMethodEnum): Promise<Mail> {
		return this._mailFacade.createDraft(this.getSubject(), body,
	...
	async send(
		...
		const doSend = async () => {
			...
			await this._mailFacade.sendDraft(neverNull(this._draft), this.allRecipients(), this._selectedNotificationLanguage)
